#!/usr/bin/env python3
"""Step 4: AI 分类 — 用 Gemini Flash 批量分类有文稿的视频。
使用 urllib（零外部依赖）。

输入: data/raw/videos.json + data/transcripts/*.txt
输出: data/classified/classification.json
"""
...
def main():
    ...
    remaining = [b for b in transcript_bvids if b not in progress]
    total = len(transcript_bvids)
    done = len(progress)
    print(f"Total: {total}, Done: {done}, Remaining: {len(remaining)}")

    if not remaining:
        print("All classified!")
        write_output(all_videos, progress)
        return
    ...
    for i, bvid in enumerate(remaining):
        ...
        # Read transcript (first 1500 chars)
        tpath = os.path.join(TRANSCRIPTS_DIR, f"{bvid}.txt")
    ...
    write_output(all_videos, progress)
    print(f"\nDone! OK: {total - errors}, Errors: {errors}")


def write_output(all_videos, progress):
    results = []
    for bvid, cls in progress.items():
        if "error" in cls:
            continue
    ...
    with open(OUTPUT_FILE, "w", encoding="utf-8") as f:
        json.dump({"meta": {"total": len(results), "classified_at": time.strftime("%Y-%m-%dT%H:%M:%S"), "model": MODEL}, "videos": results}, f, ensure_ascii=False, indent=2)
